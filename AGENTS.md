...
- Keep dataset provenance, checkpoint lineage and source/component licenses explicit. Do not commit large data, secrets or private machine configuration.
- Use original procedural or Blender-created fly assets; verify license and provenance before reusing third-party samples. Do not infer that a humanoid avatar protocol already supports a fly body.
- Test meaningful boundary behavior and numerical/state contracts. Document negative results. Commit messages serve as the initial changelog; do not create release-note machinery without a need.
