...
export function createConnectomeHttp({service,readBody,json,checkOrigin}) {
  return async function handle(request,response,url,base) {
    ...
    const match=/^\/api\/connectomes\/([0-9a-f-]+)(?:\/(commands|history))?$/.exec(url.pathname);
    ...
    if(request.method==='GET') {
      if(operation==='commands')throw new RuntimeError('Use POST for research commands.',405);
      json(response,200,id?(operation==='history'?service.history(id):service.snapshot(id)):service.view());return true;
    }
    if(request.method!=='POST'||id&&operation!=='commands')throw new RuntimeError('Method not allowed.',405);
    checkOrigin(request,base);const body=await readBody(request);
    json(response,200,id?await service.command(id,body):await service.create(body));return true;
  };
}
