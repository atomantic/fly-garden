...
import { openCrossCatalogJournal, validateCrossCatalogJournalDocument } from './cross-catalog-checkpoint.js';
const hash = b=>createHash('sha256').update(b).digest('hex');
const exact=(v,keys)=>v&&typeof v==='object'&&!Array.isArray(v)&&Object.keys(v).length===keys.length&&keys.every(k=>Object.hasOwn(v,k));
const allowed=name=>name==='fixture.json'||name==='connectomes/catalog.json'||/^connectomes\/checkpoints\/[a-f0-9-]{36}\.json$/.test(name)||/^(recordings|connectome-recordings)\/[a-f0-9-]{36}\.json$/.test(name)||name==='cross-catalog/journal.json';
...
function list(root,prefix=''){
  const out=[];for(const name of readdirSync(join(root,prefix))){const rel=prefix?`${prefix}/${name}`:name,s=lstatSync(join(root,rel));if(s.isSymbolicLink())throw new Error('Symlink in backup');if(s.isDirectory()){if(!['connectomes','connectomes/checkpoints','recordings','connectome-recordings','cross-catalog'].includes(rel))throw new Error('Unexpected backup directory');out.push(...list(root,rel));}else if(s.isFile())out.push(rel);else throw new Error('Non-file backup entry');if(out.length>4300)throw new Error('Backup file count bound exceeded');}return out.sort();
}
...
function crossCatalog(source){
  const root=join(source,'cross-catalog');
  if(!present(root))return null;
  directory(root);
  const path=join(root,'journal.json');
  if(!present(path))return null;
  readBackupBytes(path,2*1024*1024);
  const journal=openCrossCatalogJournal(root);
  try{return validateCrossCatalogJournalDocument(journal.document());}finally{journal.close();}
}
...
export function backupApplication(source,destination,{profiles=portableConnectomeProfiles()}={}){
  ...
  try{
    ...
    if(hasConnectomes){directory(c);readBackupBytes(join(c,'catalog.json'),2*1024*1024);store=openConnectomeStore(c,{profiles});}
     ...
     const journal=crossCatalog(source);
    ...
    if(hasRecordings){mkdirSync(join(destination,'recordings'),{mode:0o700});for(const value of recordings)write(join(destination,'recordings',`${value.session.id}.json`),Buffer.from(JSON.stringify(value)));sync(join(destination,'recordings'));}
     ...
     if(journal){mkdirSync(join(destination,'cross-catalog'),{mode:0o700});write(join(destination,'cross-catalog','journal.json'),Buffer.from(JSON.stringify(journal)));sync(join(destination,'cross-catalog'));}
     let total=0;const files=list(destination).map(path=>{if(!allowed(path))throw new Error('Unexpected backup file');const bytes=readBackupBytes(join(destination,path),path.startsWith('connectome-recordings/')?SAMPLE_EXPORT_MAX:path.startsWith('cross-catalog/')?2*1024*1024:32*1024*1024);total+=bytes.length;if(total>MAX_TOTAL)throw new Error('Backup total bound exceeded');return{path,bytes:bytes.length,sha256:hash(bytes)};});
     const schemaVersion=journal?3:hasConnectomeRecordings?2:1;
     const components={fixture:true,connectomes:hasConnectomes,recordings:hasRecordings};
     if(hasConnectomeRecordings)components.connectomeRecordings=true;
     if(journal){components.connectomeRecordings=hasConnectomeRecordings;components.crossCatalog=true;}
     const data={schemaVersion,kind:'fly-garden-application-backup',createdAt:new Date().toISOString(),components,files};
    write(join(destination,'manifest.json'),Buffer.from(JSON.stringify({...data,sha256:hash(JSON.stringify(data))})));validateApplicationBackup(destination,{profiles});sync(destination);sync(dirname(destination));
     return{status:'backed-up-offline',individualCount:saved.data.identities.individuals.length,connectomeCount:store?.identities().length??0,recordingCount:recordings.length,connectomeRecordingCount:connectomeRecordings.length,crossCatalog:!!journal};
...
export function validateApplicationBackup(source,{profiles=portableConnectomeProfiles()}={}){
  ...
  const componentKeys=m?.schemaVersion===1?['fixture','connectomes','recordings']:m?.schemaVersion===2?['fixture','connectomes','recordings','connectomeRecordings']:m?.schemaVersion===3?['fixture','connectomes','recordings','connectomeRecordings','crossCatalog']:[];
  if(!exact(m,['schemaVersion','kind','createdAt','components','files','sha256'])||![1,2,3].includes(m.schemaVersion)||m.kind!=='fly-garden-application-backup'||typeof m.createdAt!=='string'||!Number.isFinite(Date.parse(m.createdAt))
    ||!exact(m.components,componentKeys)||m.components.fixture!==true||!componentKeys.slice(1).every(k=>typeof m.components[k]==='boolean')||!Array.isArray(m.files)||m.files.length>4300)throw new Error('Incompatible application backup manifest');
  ...
  for(const f of m.files){if(!exact(f,['path','bytes','sha256'])||typeof f.path!=='string'||!allowed(f.path)||paths.has(f.path)||!Number.isSafeInteger(f.bytes)||f.bytes<1||f.bytes>(f.path.startsWith('connectome-recordings/')?SAMPLE_EXPORT_MAX:f.path.startsWith('cross-catalog/')?2*1024*1024:32*1024*1024)||typeof f.sha256!=='string'||!/^[a-f0-9]{64}$/.test(f.sha256))throw new Error('Invalid backup file reference');paths.add(f.path);total+=f.bytes;if(total>MAX_TOTAL)throw new Error('Backup total bound exceeded');}
  ...
  if(!m.components.crossCatalog&&[...paths].some(p=>p.startsWith('cross-catalog/')))throw new Error('Unexpected cross-catalog component');
  ...
  if(existsSync(join(source,'cross-catalog'))!==!!m.components.crossCatalog)throw new Error('Cross-catalog component directory mismatch');
  if(m.components.crossCatalog){directory(join(source,'cross-catalog'));validateCrossCatalogJournalDocument(JSON.parse(readBackupBytes(join(source,'cross-catalog','journal.json'),2*1024*1024)));}
  ...
  if(connectomeRecordings.length>100||connectomeRecordings.reduce((n,r)=>n+r.session.nextSequence,0)>10000||connectomeRecordings.reduce((n,r)=>n+r.records.reduce((s,v)=>s+Buffer.byteLength(JSON.stringify(v)),0),0)>32*1024*1024)throw new Error('Connectome recording aggregate bound exceeded');
   return{manifest:m,fixture:saved,recordings,connectomeRecordings,crossCatalog:m.components.crossCatalog?JSON.parse(readBackupBytes(join(source,'cross-catalog','journal.json'),2*1024*1024)):null};
}
...
export function restoreApplicationBackup(source,destination,{profiles=portableConnectomeProfiles(),syncDirectory=sync}={}){
  ...
  try{
    ...
    if(value.manifest.components.crossCatalog){mkdirSync(join(destination,'cross-catalog'),{mode:0o700});write(join(destination,'cross-catalog','journal.json'),Buffer.from(JSON.stringify(value.crossCatalog)));syncDirectory(join(destination,'cross-catalog'));}
