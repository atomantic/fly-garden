...
const allowed=name=>name==='fixture.json'||name==='connectomes/catalog.json'||/^connectomes\/checkpoints\/[a-f0-9-]{36}\.json$/.test(name)||/^recordings\/[a-f0-9-]{36}\.json$/.test(name);
const MAX_TOTAL=1100*1024*1024;
...
export function backupApplication(source,destination,{profiles=portableConnectomeProfiles()}={}){
  ...
  try{
    const saved=fixture(source),c=join(source,'connectomes'),r=join(source,'recordings');
    const hasConnectomes=present(c),hasRecordings=present(r);
    ...
    let total=0;const files=list(destination).map(path=>{if(!allowed(path))throw new Error('Unexpected backup file');const bytes=readBackupBytes(join(destination,path),32*1024*1024);total+=bytes.length;if(total>MAX_TOTAL)throw new Error('Backup total bound exceeded');return{path,bytes:bytes.length,sha256:hash(bytes)};});
    const data={schemaVersion:1,kind:'fly-garden-application-backup',createdAt:new Date().toISOString(),components:{fixture:true,connectomes:hasConnectomes,recordings:hasRecordings},files};
    ...
    return{status:'backed-up-offline',individualCount:saved.data.identities.individuals.length,connectomeCount:store?.identities().length??0,recordingCount:recordings.length};
...
export function validateApplicationBackup(source,{profiles=portableConnectomeProfiles()}={}){
  ...
  if(!exact(m,['schemaVersion','kind','createdAt','components','files','sha256'])||m.schemaVersion!==1||m.kind!=='fly-garden-application-backup'||typeof m.createdAt!=='string'||!Number.isFinite(Date.parse(m.createdAt))
    ||!exact(m.components,['fixture','connectomes','recordings'])||m.components.fixture!==true||!['connectomes','recordings'].every(k=>typeof m.components[k]==='boolean')||!Array.isArray(m.files)||m.files.length>4300)throw new Error('Incompatible application backup manifest');
  ...
  for(const f of m.files){if(!exact(f,['path','bytes','sha256'])||typeof f.path!=='string'||!allowed(f.path)||paths.has(f.path)||!Number.isSafeInteger(f.bytes)||f.bytes<1||f.bytes>32*1024*1024||typeof f.sha256!=='string'||!/^[a-f0-9]{64}$/.test(f.sha256))throw new Error('Invalid backup file reference');paths.add(f.path);total+=f.bytes;if(total>MAX_TOTAL)throw new Error('Backup total bound exceeded');}
  ...
  return{manifest:m,fixture:saved,recordings};
}
...
export function restoreApplicationBackup(source,destination,{profiles=portableConnectomeProfiles(),syncDirectory=sync}={}){
  ...
  return{status:'restored-offline',individualCount:value.fixture.data.identities.individuals.length,recordingCount:value.recordings.length,simulationStarted:false};
}
