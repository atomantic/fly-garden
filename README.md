...
In **Connectome lab**, create a saved, unloaded individual, then choose **Load complete graph (paused)**. Loading requires matching local memory evidence and enough configured capacity. **Start** only permits an explicit bounded **Advance**; it does not begin background execution. Checkpoint history preserves exact source lineage. The separate [shared full-connectome research barrier](docs/CONNECTOME_SHARED_RESEARCH.md) can explicitly synchronize already loaded graphs in fixed 5 ms barriers and persist/restore a catalog-local full-connectome joint checkpoint while remaining paused; saved restore is selectable independently of a live shared session. The shared barrier itself has no rendered body, sensory input, motor output, retained learning, or live heterogeneous cross-catalog coordination. The [cross-catalog coordinator](docs/CROSS_CATALOG_CHECKPOINTS.md) now has real per-member adapters, live-registry worker hooks, service reservations, local HTTP routes, a paused lab recovery panel and backup coverage. A separate [render-only mixed world shell](docs/MIXED_WORLD_SHELL.md) observes fixture and research sessions side by side. Research participants appear only as labeled markers, and opening the shell changes no one. See [the full-graph service and controls](docs/CONNECTOME_HTTP.md).
