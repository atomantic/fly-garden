...
Delivered at `631e7ab`: configurable capacity with a default of one, paused residents counted, and a lowered limit that stops new admissions without evicting or resetting anyone; a shared fixed-step fixture session joining 2–64 admitted synthetic runtimes with atomic barriers, joint checkpoints and per-recipient encounter adapters; and opt-in version 2 sessions in which one member may rest or withdraw at a recorded boundary while the world continues for the others. The current branch adds a separate [full-connectome research barrier](docs/CONNECTOME_SHARED_RESEARCH.md) for already loaded MaleCNS/BANC workers: complete 5 ms barriers, five exact 1 ms substeps per active graph, completion-order-independent commits, rollback on a reported failure, owner-scoped lifecycle controls, and a catalog-local full-connectome joint checkpoint save/paused restore with independent saved-membership selection. It also exposes observation-only barrier queue/lag/completion-order telemetry with bounded history, and an explicit bounded zero-drive shared measurement whose validated capacity stays null unless it matches the pinned full graphs and this runtime. No complete pinned pair has been measured through it yet. A [render-only mixed world shell](docs/MIXED_WORLD_SHELL.md) lists and draws fixture and research sessions side by side with explicit provenance: fixture bodies at engineered poses, and research participants only as markers without body, sensory or motor coupling. A versioned, default-off [shared environment adapter contract](docs/SHARED_ENVIRONMENT_ADAPTER.md) sets out declared sensory proxies and bounded motor proposals. It covers fail-closed capability declarations, complete batches committed in membership order, and pauses on stale, partial or faulting input. It is tested only with tiny graphs and synthetic doubles; full-connectome workers declare no channel through it. Neither the barrier nor the shell is a coupled mixed garden, sensory/body adapter, heterogeneous cross-catalog coordinator, active-input pair measurement, or biological result. Still planned: a shared session containing a full-connectome worker with declared sensory/motor coupling, explicit per-backend substeps in a rendered mixed session, and integrated active-pair resource measurement. The zero-drive pair envelope measured so far is two independently owned workers, not a coupled world.
