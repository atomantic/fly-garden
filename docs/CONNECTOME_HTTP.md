...
## Heterogeneous cross-catalog transactions

The local service exposes a separate, explicit transaction path at `GET /api/cross-catalog`, `GET /api/cross-catalog/checkpoints`, `POST /api/cross-catalog`, `POST /api/cross-catalog/restore` and `POST /api/cross-catalog/recover`. The fixed namespaces are `fixture`, `connectome:male` and `connectome:banc`; browser input supplies only stable IDs and namespaces. Save requires an exact mixed membership of 2–64 explicitly paused members, a fixed 5 ms interval and a non-negative tick. Restore uses the complete saved membership and namespace set. Recovery accepts only the reported transaction ID and an explicit `rollback` or `complete` action.

The service reserves fixture and research participants before re-reading their heads, drains queued registry work, preflights both catalog families, stages immutable payloads and publishes selected research heads to the live registry before activation. All activation is paused with fresh epochs; active/resting modes are preserved. The lab panel never auto-recovers, never retries and disables overlapping operations. The journal is local, checksummed and included in explicit offline backup. This path has no body, sensory input, motor output, retained learning or biological validation, and it does not change the separate catalog-local shared barrier.
