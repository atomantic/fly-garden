...
The checkpoint UI serves the primary resident. The registry can explicitly create a fresh fixture identity or a research replica from a checkpoint. Replicas have new individual IDs and retain source individual/checkpoint lineage. Both start **saved-unloaded**. Explicit load constructs a paused runtime with a fresh session; repeated load of a resident leaves it unchanged. Explicit unload saves before releasing the runtime; failed saves retain the resident in a fault pause. Startup restores only the primary resident paused; other saved individuals remain unloaded.
