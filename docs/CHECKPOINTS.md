...
The checkpoint UI serves the primary resident. The API can create a fresh fixture or an explicitly labeled research replica from a checkpoint. Replicas have new individual IDs and retain source individual/checkpoint lineage; fresh individuals have no replica parent. Both initially stay **saved-unloaded** and cannot run or receive encounters.

The registry has a programmatic integration seam: `openIdentityStore(directory, { residentIds: [savedId, ...] })` selects additional already-saved fixtures alongside the primary. The default list is empty, so the production entrypoint still loads only the primary. Selection validates the entire store and every requested ID before returning any live runtime. Every selected resident has its own dynamics, input policy, fault and command session, and starts paused. There is no pair-specific limit. Checkpoint storage limits still apply.

This seam implements state isolation, **not resource admission**. It is for the admission layer and tests; HTTP creation cannot request residency and does not allocate another running brain. Configured budgets, live load/unload and a population UI belong to #22/#3. Do not bypass unknown resource capacity by passing arbitrary resident lists for real datasets. No dataset workers are accepted by this fixture store. Its independent `step()` calls are not a coupled-world synchronization barrier; shared-world clocks belong to #20.
...
Automated tests cover exact fixture neural round trips, paused fresh-session recovery, canceled inputs with retained reservations, corrupt/version rejection, failed-write rollback, replica isolation, writer exclusion and stale-command rejection. Three-resident fixture tests verify that interleaved controls, save/restore and a targeted persistence failure leave unrelated residents unchanged. HTTP tests exercise crossed IDs/sessions, duplicate creation and multiple observers. Namespace corruption is rejected even after a payload checksum is recomputed. No real dataset restore, cross-dataset dynamics, learned-state retention, visual causal control or multi-worker capacity has been validated by these tests.
