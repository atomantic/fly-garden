...
The connectome catalog now supports a schema-2 joint record for a full-connectome-only shared session. A paused explicit save validates every member checkpoint and replaces all selected heads and the joint record in one catalog commit; restore appends explicit restore lineage and returns fresh paused epochs. A pre-commit failure leaves prior heads selected, and a post-rename durability uncertainty remains unavailable until the catalog is reopened. The fixture identity catalog is still separate, so this is not yet a heterogeneous fixture/connectome cross-store coordinator.
...
The targeted regression tests use tiny named graphs, temporary catalogs and injected worker scheduling. They do not load the pinned full datasets, measure production pair capacity, run a simulation automatically, or establish a biological result.
