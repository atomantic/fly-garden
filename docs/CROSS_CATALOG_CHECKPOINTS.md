...
- `rollback` first re-issues any interrupted `reverting` compensation. Idempotence makes a lineage head appended just before a crash explainable. It then reverts every catalog that selected a planned head, cancels transaction-owned staging, and (for restore) evicts all members. The record ends `rolled-back`.
