...
Bounded connection samples and paginated incoming/outgoing inspection are now available through an explicit connectivity read; see [connectivity and browser evidence](ATLAS_CONNECTIVITY.md). Live and recorded-replay activity is now a separate, explicitly chosen, bounded overlay with its own counts and provenance; see [activity modes](ATLAS_ACTIVITY_MODES.md). It is capped at the kernel's 256-cell sample limit, so it is never a whole-network activity view, and an unsampled or unavailable cell is never shown as zero activity. Anatomical axis directions, reconstructed neurites and full peripheral coverage remain unclaimed.
